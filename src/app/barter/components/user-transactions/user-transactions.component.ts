...
export class UserTransactionsComponent implements OnInit, AfterViewInit {
  ...
  elements: BehaviorSubject<TransactionAndUsers[]>;
  ...
  constructor(private cdRef: ChangeDetectorRef,
    ...
    this.elements = new BehaviorSubject<TransactionAndUsers[]>([]);
    this.originalElements = [];
  }

  ngOnInit() {
    let elementsToInsert: TransactionAndUsers[] = [];
    ...
    this.activatedRoute.data.subscribe((result) => {
      result.data.forEach((transaction: TransactionAndUsers) => {
        elementsToInsert.push({
          ...
          traderItem: transaction.traderItem,
          ownerItem: transaction.ownerItem,
          status: transaction.status,
          offerDate: transaction.offerDate,
          transactionCompleteDate: transaction.transactionCompleteDate,
      ...
      this.originalElements = [...elementsToInsert];
  ...
  declineOffer(transaction): void {
    this.transactionStore$.select(getTransaction(transaction.id)).pipe(first())
      .subscribe((transactionToUpdate: Transaction) => {
        ...
        this.elements.value.find(element => element.id === transaction.id).transactionCompleteDate = firebase.firestore.Timestamp.fromDate(new Date());
        this.transactionStore$.dispatch(updateTransaction({ transaction: updatedTransaction }));
  ...
  myOffersFilter(): void {
    ...
    if (this.myOffersCheck) {
      this.elements.next(this.elements.value.filter(transaction => transaction.trader.id === this.currentUser.id));
    }
